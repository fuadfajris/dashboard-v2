...
const formDetailMap: Record<string, any> = {
  ...
  lineup: dynamic(() => import("@/components/form/detail/LineupFormDetail")),
};
    ...
    const fetchActivity = async () => {
      ...
      try {
        ...
        const parsedMaker: Maker | null = data.maker
          ...
          : null;
        const parsedApprover: Maker | null = data.approver
  ...
  const handleStatusChange = async (newStatus: "approved" | "rejected") => {
    ...
    try {
      ...
      if (newStatus === "approved") {
        ...
        const isEdit = !!parsedContent.prevValue && !!parsedContent.newValue;
        const isAdd = !parsedContent.prevValue && !!parsedContent.newValue;
        const isDelete =
          !!parsedContent.prevValue && parsedContent.newValue === null;
        let method;
        if (isAdd) {
          method = "POST";
        } else if (isEdit) {
        ...
        switch (activity.content_key) {
          ...
          case "lineup":
            let urlLineup;
            if (isAdd) {
              urlLineup = `${templateUrl}/guests/lineup`;
            } else if (isEdit) {
              urlLineup = `${templateUrl}/guests/lineup/${
                parsedContent.newValue.id || parsedContent.prevValue.id
              }`;
            } else if (isDelete) {
              urlLineup = `${templateUrl}/guests/lineup/${parsedContent.prevValue.id}`;
            }

            const reqLinup = isDelete
              ? activity.content.prevValue
              : activity.content.newValue;

            await fetch(`${urlLineup}`, {
              method: method,
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
              },
              body: JSON.stringify(reqLinup),
            });
            break;
          default:
            break;
